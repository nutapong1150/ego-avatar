...
> **ขอบเขต:** สกิลนี้คุม *กลยุทธ์ / copy / conversion / honesty* · ส่วน *การลงมือทำให้เสร็จจริง* (เตรียม asset → mockup → โครง CSS → ตรวจด้วยการวัด → ขึ้น prod) อยู่ที่สกิล **`landing-page-build`** ใช้คู่กันเสมอ
...
1. **1:1 Attention Ratio:** 1 หน้า ต้องมีเพียง 1 เป้าหมาย 1 เส้นทาง และ 1 Call to Action (CTA) เท่านั้น
   - สิ่งที่ต้องตัดจริงๆ คือ **ลิงก์ที่พาผู้ใช้ออกไปจากหน้านี้** (เมนูไปหน้าอื่น ลิงก์ภายนอก ลิงก์โซเชียล)
   - **ลิงก์กระโดดภายในหน้าเดียวกัน (anchor เช่น วิธีใช้/ราคา/คำถาม) ไม่ผิดกฎ** เพราะยังรั้งคนไว้ในหน้าเดิม และช่วยคนที่มาหาข้อมูลเฉพาะจุด — หน้ายาวหลายบล็อกควรมี
   - **footer ต้องมีลิงก์นโยบาย/ข้อกำหนด/ความปลอดภัย** เป็นข้อกำหนดตามกฎหมายและเป็นตัวสร้างความน่าเชื่อถือ ไม่ใช่ตัวขโมยโฟกัส (ทำให้จางลงได้ แต่ห้ามไม่มี)
...
[LOCK — ห้ามเปลี่ยน]
• กลุ่มเป้าหมาย: [ใคร]
• โครง section เรียงเป๊ะตามนี้: hero → [urgency banner ถ้าจริง] → ปัญหา(pain) → ฟีเจอร์→ประโยชน์ → [จอจำลอง] → 3 ขั้นเริ่มใช้ → [ราคา] → CTA ปิดท้าย → footer
...
5. **จอจำลอง (mock screen) ประกอบด้วย HTML/CSS** — หลักฐานว่าใช้ได้จริง (แทน social proof ที่ยังไม่มี) · ⚠️ **อย่าใช้สกรีนช็อตจากระบบจริง** เพราะมักติดชื่อคน/รูปโปรไฟล์/ข้อความจริง = ข้อมูลส่วนบุคคลบนเว็บสาธารณะ ขัดกับนโยบาย PDPA ที่ตัวเองประกาศ · จอจำลองยังอ่านออกกว่าบนมือถือ แก้ง่ายกว่า ไฟล์เบากว่า · **เส้นที่ห้ามข้าม: ถ้อยคำต้องตรงกับที่ระบบส่งจริง 100%**
...
- [ ] **Trust เชิงฟังก์ชัน** — เมื่อยังไม่มีรีวิว: ใช้จอจำลองที่ถ้อยคำตรงกับระบบจริง + niche positioning ("ออกแบบเพื่อ[กลุ่ม]ไทยโดยเฉพาะ") + จุดความปลอดภัย (เช่น "ระบบไม่ถือเงิน", "ตาม PDPA")
